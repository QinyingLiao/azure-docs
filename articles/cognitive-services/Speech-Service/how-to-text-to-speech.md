...
The body of the POST request for Text to Speech may be plain text (ASCII or UTF8) or an [SSML](speech-synthesis-markup.md) document. Plain-text requests are spoken with a default voice. In most cases, you want to use an SSML body. The HTTP request must include an [authorization token](https://docs.microsoft.com/en-us/azure/cognitive-services/speech-service/rest-apis#authentication). 
